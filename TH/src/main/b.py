...
def get_max_area(img_in, contours, name):
    ...
    for idx, area in sorted(enumerate([cv2.contourArea(x) for x in contours]), key=lambda x: x[1], reverse=True):
        ...
        print(extBot, extLeft, extTop, extRight)
        # print(extLeft - 10, extTop - 10, extRight - extLeft + 20, extBot - extTop + 20)
        cropped_img = img_trim(img_in, extLeft[0] - 10, extTop[1] - 10, extRight[0] - extLeft[0] + 20, extBot[1] - extTop[1] + 20)
