...
def is_triangle(_img):
    img2 = cv2.GaussianBlur(_img, (3, 3), 0)
...
def houghCircle(img1):
    ...
    if circles is not None:
        ...
        for i in circles[0, :]:
            ...
            # cv2.imshow("triangle", cv2.resize(img1, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC))
            # cv2.imshow("triangle", cv2.resize(img1, (200,200)))
            return True
    return False
    ...
    for idx, area in sorted(enumerate([cv2.contourArea(x) for x in contours]), key=lambda x: x[1], reverse=True):
        ...
        if extLeft[0] < 10 or extTop[1] < 10:
            ...
            cropped_img = img_trim(img_in, extLeft[0] - 10, extTop[1] - 10, extRight[0] - extLeft[0] + 20,
                                   extBot[1] - extTop[1] + 20)
        if houghCircle(cropped_img) == True:
            cv2.drawContours(img_out, [_contour], 0, (0, 255, 100), 2)
            myList.append(cropped_img)
            break
        elif is_triangle(cropped_img) == True:
            cv2.drawContours(img_out, [_contour], 0, (0, 255, 100), 2)
...
def get_red_color(frame, name):
    ...
    Hmask = Hmask1 | Hmask2
    Ymask = cv2.inRange(yuv, np.array([0, 130, 130]), np.array(
        [80, 225, 225]))  # yellow + red : np.array([20 or 30, 225, 255])  # only red : np.array([10, 225, 255])
...
def convert_to_tflist(_list):
    newlist = []
    for e in _list:
        newlist.append(cv2.imencode('.jpg', e)[1].tostring())
    return newlist


if __name__ == "__main__":
    cv2.namedWindow("triangle", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("triangle", 200, 100)
    test.sess = tf.Session()
    test.create_graph()
    ...
    while cap.isOpened():
        ...
        upper_frame = get_red_color(_trim_upper, "ur")
        left_frame = get_red_color(_trim_left, "lr")
        right_frame = get_red_color(_trim_right, "rr")

        myList = []
        upper, upper_list = get_max_area(_trim_upper, upper_frame)
        left, left_list = get_max_area(_trim_left, left_frame)
        right, right_list = get_max_area(_trim_right, right_frame)
        myList = upper_list + left_list + right_list
        if len(myList) != 0:
            tf_list = convert_to_tflist(myList)
            answer = test.run_inference_on_image(tf_list)
            print("Detection - ", answer)
            cv2.putText(myList[0], answer, (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.imshow("triangle", myList[0])
